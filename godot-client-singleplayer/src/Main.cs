...
public partial class Main : Node
{
        ...
        {
                GD.Print("LastLight Godot Single-Player Starting...");
                ...
                _hud.Visible = false;
                AddChild(_hud);
                _hud.SwapItemRequested += (fromIndex, toIndex) => _server.HandleSwapItem(new SwapItemRequest { FromIndex = fromIndex, ToIndex = toIndex });
                _hud.UseItemRequested += (slotIndex) => _server.HandleUseItem(new UseItemRequest { SlotIndex = slotIndex });
                ...
                _server.Name = "LocalServer";
                AddChild(_server);

                // Connect Signals
                _server.JoinResponseReceived += OnJoinResponse;
                _server.WorldInitReceived += OnWorldInit;
                _server.PlayerUpdateReceived += OnPlayerUpdate;
                _server.BulletSpawned += OnBulletSpawned;
                _server.BulletHit += OnBulletHit;
                _server.EnemySpawned += OnEnemySpawned;
                _server.EnemyUpdated += OnEnemyUpdated;
                _server.EnemyDied += OnEnemyDied;
                _server.SpawnerSpawned += OnSpawnerSpawned;
                _server.SpawnerUpdated += OnSpawnerUpdated;
                _server.SpawnerDied += OnSpawnerDied;
                _server.BossSpawned += OnBossSpawned;
                _server.BossUpdated += OnBossUpdated;
                _server.BossDied += OnBossDied;
                _server.PortalSpawned += OnPortalSpawned;
                _server.PortalDied += OnPortalDied;
                _server.LeaderboardUpdated += OnLeaderboardUpdated;
        ...
        private void HandleShooting(float dt)
        {
                ...
                if (Input.IsMouseButtonPressed(MouseButton.Left) && _shootTimer >= _shootInterval)
                {
                        ...
                        _server.HandleFire(new FireRequest 
                        { 
                                BulletId = 0,
        ...
        private void OnWorldInit(int seed, int width, int height, int tileSize, int style, float cleanupTimer)
        {
                ...
                foreach (var enemy in _enemies.Values) enemy.QueueFree(); _enemies.Clear();
                foreach (var spawner in _spawners.Values) spawner.QueueFree(); _spawners.Clear();
                foreach (var boss in _bosses.Values) boss.QueueFree(); _bosses.Clear();
                foreach (var portal in _portals.Values) portal.QueueFree(); _portals.Clear();
                foreach (var bullet in _bullets.Values) bullet.QueueFree(); _bullets.Clear();

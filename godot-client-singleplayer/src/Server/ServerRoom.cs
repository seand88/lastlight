...
public class ServerRoom
{
    ...
    public ServerRoom(int id, RoomData data, int seed, LocalServer server, Dictionary<int, AuthoritativePlayerUpdate> allPlayers)
    {
        Id = id; Data = data; Name = data.Name; Seed = seed; Style = data.Style;
        _server = server; _allPlayers = allPlayers;
        ...
        Bosses.OnBossDied += (b) => {
            ...
            foreach (var p in GetPlayersInRoom().Values) {
                ...
                _server.SavePlayer(p.PlayerId);
            }
            
            ForceCleanupTimer = 60f;
        };
        ...
        if (ForceCleanupTimer.HasValue) {
            ...
            if (ForceCleanupTimer.Value <= 0) {
                ...
                foreach (var p in GetPlayersInRoom().Keys) {
                    _server.SavePlayer(p);
                    _server.SwitchPlayerToNexus(p);
                }
            }
        }
    ...
    public void Broadcast<T>(T packet) where T : class, new()
    {
        _server.BroadcastPacket(packet, Id);
    }
